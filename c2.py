...
def update2(id, cat, parent, title):
    try:
        ...
        if (t[1] != cat) or (t[2] != parent) or (t[3] != title):
            db.execute('update forums set category=?, parent=?, title=?, last_updated=datetime("now", "localtime") where id=?',(cat, parent, title, id,))
            db.commit()
            print('  [update]: ' + str(t))
...
def update3(id, cat, title):
    try:
        ...
        if (t[1] != cat) or (t[3] != title):
            db.execute('update forums set category=?, title=?, last_updated=datetime("now", "localtime") where id=?',(cat, title, id,))
            db.commit()
            print('  [update]: ' + str(t))
