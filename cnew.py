    ...
    for link in doc.select('a'):
        ...
        if 'f' in ps:
            if int(ps['f'][0]) not in fs:
                ...
                db.execute('insert into forums(id, title, subscribed, last_updated, last_scanned_page) values(?, ?, 1, datetime("now", "-1 year"), 1)', (ps['f'][0], link.text.strip(),))
                db.commit()
                db.execute('insert into forums_rss(id, delta, last_scanned) values(?, 3600, datetime("now", "-1 year"))', (ps['f'][0],))
                db.commit()
                fs.remove(ps['f'][0])
